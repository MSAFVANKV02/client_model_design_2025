      ...
      <div  className={`relative flex select-none items-start justify-start w-full space-y-4 ${isLastCard ? "flex-row-reverse items-start  justify-around" : ""}`}>
        ...
        <div className={`h-full  ${isTrue ? "sm:translate-y-10" :"sm:-translate-y-14"} `}>
          <img
            src={image}
            alt={title}
            className={`w-32 h-[250px] object-contain mx-auto ${isLastCard ? "mr-24" : ""}`}
            draggable={false}
          />
        </div>
...
export function CardContent({ className, ...props }: CardContentProps) {
  return (
    <div
      {...props}
      className={cn(
        "flex flex-col items-center justify-center w-full h-full p- rounded-xl shadow-lg bg-[#FDF7FF] backdrop-blur-md filter",
        className
      )}
    />
  );
